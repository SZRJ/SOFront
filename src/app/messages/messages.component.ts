...
export class MessagesComponent implements OnInit {
  ...
  fetchMessages(): void {
    this.http.get('http://4.228.231.241:3000/Messages')
      .subscribe({
        next: (messages: any) => {
  ...
  markAsRead(message: any): void {
    ...
    this.http.patch(`http://4.228.231.241:3000/Messages/${message._id}`, { readDate: updatedMessage.readDate })
      .subscribe({
        next: () => {
  ...
  deleteMessage(message: any): void {
    if (confirm('¿Estás seguro de que deseas eliminar este mensaje?')) {
      this.http.delete(`http://4.228.231.241:3000/Messages/${message._id}`)
        .subscribe({
          next: () => {
